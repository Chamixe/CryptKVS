...
int ckvs_client_compute_masterkey(struct ckvs_memrecord *mr, const struct ckvs_sha *c2)
{
    ...
    unsigned int size;
    unsigned char *l = HMAC(EVP_sha256(), mr->c1.sha, SHA256_DIGEST_LENGTH, c2->sha, SHA256_DIGEST_LENGTH, mr->master_key.sha, &size);
...
/**
 * @brief Creates the required auth_key and c1
 * 
 * @param stretched_sha 
 * @param message 
 * @param key_sha 
 * @return int 
 */
static int hmac(unsigned char *stretched_sha, const char* message, unsigned char* key_sha){
    unsigned int size_a;
    unsigned char *PA = HMAC(EVP_sha256(), stretched_sha, SHA256_DIGEST_LENGTH, message, strlen(message), key_sha, &size_a);
    if(PA == NULL || size_a != (unsigned int)SHA256_DIGEST_LENGTH){
        free(size_a);
        size_a = NULL;
        return ERR_INVALID_COMMAND;
    }
...
int ckvs_client_encrypt_pwd(struct ckvs_memrecord *mr, const char *key, const char *pwd)
{
    ...
    bzero(mr, sizeof(mr));
    const unsigned char *tab = calloc(strlen(key) + 2 + strlen(pwd), sizeof(char));
    if (tab == NULL)
    {
    ...
    SHA256(tab, strlen(tab), mr->stretched_key.sha);
    free(tab);

    hmac(mr->stretched_key.sha, AUTH_MESSAGE, mr->auth_key.sha);

    hmac(mr->stretched_key.sha, C1_MESSAGE, mr->c1.sha);
